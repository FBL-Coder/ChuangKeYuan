...
public class DesignActivity extends Activity implements OnClickListener {
    ...
    private void initListView() {
        ...
        int ID = 6;
        try {
            ID = Integer.parseInt(MyApplication.getUtil().mListBeen.get(0).getParent().get(2).getTerm_id());
        }catch (Exception e){}
        ...
        list = new AsyncLoaderData_List(this, build
                ...
                , mListView, body, "Data_build_3", count, false, ID,layout);
    ...
    private void initTitlebar() {
        ...
        try {
            mTeachTitle.setText(MyApplication.getUtil().mListBeen.get(0).getParent().get(2).getName());
        }catch (Exception e){
            mTeachTitle.setText("班级项目策划方案");
        }
        try {
            mTeach.setImageURI(Uri.parse(MyApplication.getUtil().mListBeen.get(0).getParent().get(2).getBanner()));
        }catch (Exception e){
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.iv_teach_up:
                startActivity(new Intent(DesignActivity.this, UploadActivity.class).putExtra("id","6"));
