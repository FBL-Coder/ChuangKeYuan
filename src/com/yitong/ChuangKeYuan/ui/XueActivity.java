 ...
 * 学材页面
 */
public class XueActivity extends Activity implements OnClickListener {
    ...
    protected void onRestart() {
        if (MyApplication.getFLAG() == 1010) {//判断页面是否需要刷新
            //初始化ListView
            initListView();
    ...
    private void initTitlebar() {
        ...
        try {
            mDes.setText(MyApplication.getUtil().mListBeen.get(3).getParent().get(0).getParent_ParentBean().get(0).getName());
        } catch (Exception e) {
        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.iv_teach_up:
                startActivity(new Intent(XueActivity.this, UploadActivity.class).putExtra("id", getIntent().getStringExtra("id")));
                break;
        }
    }
}
